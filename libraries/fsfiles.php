...
class FSFiles {
	...
	function uploadImage($input_tag_name, $path, $size_max = 2000000, $suffix = '', $crop = 0, $crop_width = 0, $crop_height = 0) {
		if (($_FILES [$input_tag_name] ["type"] != "image/gif") && ($_FILES [$input_tag_name] ["type"] != "image/png") && ($_FILES [$input_tag_name] ["type"] != "image/jpeg") && ($_FILES [$input_tag_name] ["type"] != "image/pjpeg")) {
    ...
    */
	function cropImge($img_source, $img_destination, $crop_width = 1, $crop_height = 1) {
		...
		$cropped_tmp = @imagecreatetruecolor ( $new_width, $new_height ) or die ( "Cannot Initialize new GD image stream when cropped" );
		// transparent
    ...
    */
	function resize_image($img_source, $img_destination, $crop_width = 1, $crop_height = 1) {
		...
		$file_ext = $this->getExt ( $img_source );
		// new
		$newpic = imagecreatetruecolor ( round ( $real_width ), round ( $real_height ) ) or //		$cropped_tmp = @imagecreatetruecolor($real_width,$real_height)
		die ( "Cannot Initialize new GD image stream when cropped" );
		// transparent
    ...
    */
	function cut_image($img_source, $img_destination, $crop_width = 1, $crop_height = 1) {
		...
		$file_ext = $this->getExt ( $img_source );
		// new
		$newpic = imagecreatetruecolor ( $crop_width, $crop_height ) or die ( "Cannot Initialize new GD image stream when cropped" );
		// transparent
		imagealphablending ( $newpic, false );
		imagesavealpha ( $newpic, true );
		$transparent = imagecolorallocatealpha ( $newpic, 255, 255, 255, 127 ); //
    ...
    */
    ...
    function resize_image_fix($img_source, $img_destination, $crop_width = 1, $crop_height = 1){
    	if (!file_exists($img_source)) {
