...
class Remote_db {
    ...
    function Remote_db() {
    	$db_info_remote =  array(
					'dbName'=> 'nhasachhuongthuy',
						'dbUser' => 'root',
						'dbPass'=> '',
						'dbHost'=> 'localhost'
					);
		$this -> db_info_remote = $db_info_remote;
//        $this->db = $db_info_remote;
		if( empty( $db_info_remote['dbPort'] ) )
			$this->port = 3306;
		else 
			$this->port = $db_info_remote['dbPort'];
    }

    function connect() {
    	$db_info_remote = $this -> db_info_remote ;
        $this->conn_id = @mysql_pconnect($db_info_remote['dbHost'].":".$this->port,$db_info_remote['dbUser'],$db_info_remote['dbPass']);
        if ($this->conn_id === false)
        	 $this->sql_error("Connection Error");
        	 
        if (!@mysql_select_db($db_info_remote['dbName'], $this->conn_id))
...
//    }
    ...
    function checkExistTable($tablename)
    {
    	...
    	$sql  = "SELECT  TABLE_NAME  FROM INFORMATION_SCHEMA.TABLES WHERE 
				...
				AND TABLE_SCHEMA = '".$db_info_remote['dbName']."'";
    	$db = new Remote_db();
    	$db -> query($sql);
    	$rs = $db -> getResult();
    	if($rs)
