			...
			<table border="1" class="tbl_form_contents" width="100%">
				...
				<tbody>
					...
					<?php foreach ($list as $row) { ?>
                        ...
                        <?php 
                            $arr_param = explode(',', $add_param);
                            $arr_value = explode(',', $str_value_in_record);
                            $str_uri = $link_menu;
                            for($k = 0; $k < count($arr_param); $k ++){
                    			$str_uri .= '&'.$arr_param[$k].'='.$arr_value[$k];
                    		}
                        ?>
                        <tr class="row<?php echo $i%2; ?>">
                            <td style="text-align: left; color: red;" colspan="<?php echo 2+$j; ?>"><?php echo FSRoute::_($str_uri);?></td>
                        </tr>
						<?php $i++; ?>
					<?php }?>
...
<script type="text/javascript">
	...
	function add_param(value){
		...
		window.opener.document.getElementById("menu_link").value += str_uri;
		window.close();
	}
</script>