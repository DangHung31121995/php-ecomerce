<?php 
	class LanguagesModelsText_admin extends FSModels
	{
		var $limit;
		var $prefix ;
		...
		{
			$this -> limit = 20;
			$this -> table_name = 'fs_languages_text_admin';
			parent::__construct();
		}
		...
		function save(){
			...
			if($id){
				$rs = $this -> _update($row, 'fs_languages_text_admin','id = '.$id);
			}else{
				$rs = $this -> _add($row, 'fs_languages_text_admin');
			}
			return $rs;
		}
		
		function check_duplicate($key,$id){
			return $this -> check_exist($key,$id,'lang_key','fs_languages_text_admin');
		}
	}
