		...
		function save_new_field($table_name)
		{
			...
			if($new_field_total)
			{
					...
					if($new_fname){
						...
						if($new_fname && $this -> check_valid_name($new_fname) )	
						{
							...
							if($new_ftype == 'foreign_one' || $new_ftype == 'foreign_multi'){
								...
								$foreign = $this -> get_record_by_id($foreign_id,'fs_extends');
							...
							$arr_insert_table[]  = 		"('$table_name','$new_fname','$new_ftype','$new_fshow','$foreign_id','$foreign_name','$foreign_tablename','$is_main','$is_filter','$is_config','$group_id','$ordering')";
				...
				if(count($arr_insert_table))
				{
					...
					$sql_insert_table .= "		(table_name,field_name,field_type,field_name_display,foreign_id,foreign_name,foreign_tablename,is_main,is_filter,is_config,group_id,ordering) ";
