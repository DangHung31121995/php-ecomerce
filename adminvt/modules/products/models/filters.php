		...
		function save_filter_new($table_name,$field_name,$field_show,$field_current)
		{
			...
			if($total)
			{
				...
				for($i = 0 ; $i < $total ; $i ++ )
				{
					...
					{
						$arr_insert[]  = 		"('$filter_show','$alias','$table_name','$field_name','$field_show','$calculator','$value','$seo_title','$seo_meta_key','$seo_meta_des',$published,$is_common,'$field_ordering')";
					}
				}
				...
				if(count($arr_insert))
				{
					...
					$sql_insert .= "		(filter_show,alias,tablename,field_name,field_show,calculator,filter_value,seo_title,seo_meta_key,seo_meta_des,published,is_common,field_ordering) ";
					$sql_insert .= "		 VALUES "; 
			...
			for($i= 0 ; $i < $total ; $i++)
			{
				...
				if($filter_show_exist)
				{
					...
					if( ($filter_show_exist != $filter_show_exist_begin) || ($calculator_exist != $calculator_exist_begin) ||
						...
						($seo_title_exist != $seo_title_exist_begin) || ($seo_meta_key_exist != $seo_meta_key_exist_begin) || ($seo_meta_des_exist != $seo_meta_des_exist_begin)
					   )
					{
