...
<?php 
	class Products_filterBModelsProducts_filter
	{
...
//				$where .= ' AND is_common = 1';
//			}
			$query = ' SELECT id, filter_show, field_name,field_show,alias, calculator
...
//			}
			$query = ' SELECT *
						FROM fs_products_filters 
						WHERE published = 1 '.$where.' ORDER BY is_common DESC, field_ordering ASC';
				global $db;
			$db->query($query);
			return $result = $db->getObjectList();
		}
...
//			}
		...
		function get_category(){
            ...
            $query = "  SELECT id,name, icon, alias, icon, image, tablename, parent_id, list_parents, seo_title, seo_keyword, seo_description
